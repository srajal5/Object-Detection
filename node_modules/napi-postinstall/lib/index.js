...
async function checkAndPreparePackage(packageNameOrPackageJson, versionOrCheckVersion, checkVersion) {
    ...
    if (typeof packageNameOrPackageJson === 'string') {
        try {
            ...
            if (typeof versionOrCheckVersion !== 'string') {
                throw new TypeError(`Failed to load \`${constants_js_1.PACKAGE_JSON}\` from "${packageNameOrPackageJson}", please provide a version.`);
            }
    ...
    for (const target of targets) {
        ...
        const subpath = `${binaryPrefix}${target}.${target === constants_js_1.WASM32_WASI ? 'wasm' : 'node'}`;
        try {
